...
extension Z80 {
    func opCodeDDFD(index: Z8016BitRegister) {
        ...
        case 0x22: // ld (nn), index (UD)
            let target = nextWord()
            memory.writeWord(to: target, value: indexValue)
            memptr = target &+ 1
            ts = 20
        ...
        case 0x2A: // ld ix, (nn)
            let target = nextWord()
            writeIndex(index, value: memory.readWord(from: target))
            memptr = target &+ 1
            ts = 20
        ...
        case 0x34: // inc (index + d)
            ...
            let masks = halfCarryOverflowCalculationAdd(value: memory.read(from: displacedIndex), amount: 0x01)
            memory.write(to: displacedIndex, value: masks.value)
            F = (F & carry) | masks.halfCarryMask | masks.overflowMask | sz53(masks.value)
            ts = 23

        case 0x35: // inc (index + d)
            let displacedIndex = displacedIndex(index, displacement: next())
            let masks = halfCarryOverflowCalculationSub(value: memory.read(from: displacedIndex), amount: 0x01)
            memory.write(to: displacedIndex, value: masks.value)
        ...
        case 0x36:
            let displacedIndex = displacedIndex(index, displacement: next())
            memory.write(to: displacedIndex, value: next())
            ts = 19
        ...
        case 0x70...0x75, 0x77:
            ...
            let displacedIndex = displacedIndex(index, displacement: next())
            memory.write(to: displacedIndex, value: sourceValue)
            ts = 19
        ...
        case 0xE3: // ex (sp), index
            let temp = indexValue
            let value = memory.readWord(from: SP)
