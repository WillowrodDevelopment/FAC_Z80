...
extension Z80 {
    func opCodeDDFDCB(index: Z8016BitRegister) async {
        let disIndex = displacedIndex(index, displacement: await next())
        let opCode = await next()
        let source = opCode & 0x07
        let target = opCode >> 3
        let sourceValue = await memory.read(from: disIndex)
        var ts = 23
        let mCycles = 2
        switch target {
        case 0x00: // rlc
            ...
            await memory.write(to: disIndex, value: value)
            if source != 0x06 { writeToRegister(source, value: value) }
