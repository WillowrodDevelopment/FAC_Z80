...
extension Z80 {
    ...
    public func fetchAndExecute() {
        ...
        case 0x1A: // ld a, (de)
            A = memory.read(from: DE)
            memptr = DE &+ 1
            ts = 7
            recordDataBanked(DE, value8Bit: A)
        ...
        case 0x22: // ld (nn), hl
            let target = nextWord()
            memory.writeWord(to: target, value: HL)
            memptr = target &+ 1
            ts = 16
            recordDataBanked(target, value16Bit: HL)
        ...
        case 0x2A: // ld hl, (nn)
            ...
            memptr = address &+ 1
            ts = 16
            recordDataBanked(address, value16Bit: HL)
        ...
        case 0x32: // ld (nn), a
            let target = nextWord()
            memory.write(to: target, value: A)
            recordDataBanked(target, value8Bit: A)
        ...
        case 0x34: // inc (HL)
            let masks = halfCarryOverflowCalculationAdd(value: memory.read(from: HL), amount: 0x01)
            // memory[Int(HL)] = masks.value
            memory.write(to: HL, value: masks.value)
            recordDataBanked(HL, value8Bit: masks.value)
            F = (F & carry) | masks.halfCarryMask | masks.overflowMask | sz53(masks.value)
            ts = 11

        case 0x35:
            let masks = halfCarryOverflowCalculationSub(value: memory.read(from: HL), amount: 0x01)
            memory.write(to: HL, value: masks.value)
            recordDataBanked(HL, value8Bit: masks.value)
        ...
        case 0x36:
            let nxt = next()
            memory.write(to: HL, value: nxt)
            //recordDataBanked(HL, value8Bit: nxt)
            ts = 10
        ...
        case 0x3A: // ld A, (nn)
            let target = nextWord()
            memptr = target &+ 1
            A = memory.read(from: target)  //memory[target]
            ts = 13
            recordDataBanked(target, value8Bit: A)
        ...
        case 0x40...0x6F, 0x78...0x7F: // ld r,r
            ...
            case 0x05:
                L = sourceValue
            case 0x06:
                memory.write(to: HL, value: sourceValue)
            case 0x07:
                A = sourceValue
            default:
                break
        ...
        case 0x70...0x75, 0x77: // ld r,r
            ...
            case 0x07:
                A = sourceValue
                if source == 0x07 {
                    recordDataBanked(HL, value8Bit: A)
                }
            default:
                break
            }
        ...
        case 0xE3: // ex (sp), hl
            let temp = HL
            HL = memory.readWord(from: SP)
            memory.writeWord(to: SP, value: temp)
            memptr = HL
            ts = 19
